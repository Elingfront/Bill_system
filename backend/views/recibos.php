...
/**
 * Consulta para Recibos de Caja: Solo facturas de venta que tengan Comprobante de Ingreso (guardado en egreso_path)
 */
...
try {
    $recibos = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);
...
<style>
    ...
    .st-cobrada { color: #198754; border-color: #198754; background: #eefaf3; }
</style>

<main class="main-content">
    <div class="alt-page-header">
        <div class="alt-header-content">
            <div class="alt-header-icon"><i data-lucide="receipt" size="24"></i></div>
            <div>
                <h4 class="alt-header-title">Recibos de Caja</h4>
                <span class="alt-header-subtitle">ALT-CONFECCIONES · Ingresos de Ventas</span>
            </div>
        </div>
    ...
    <!-- Motor de Búsqueda y Filtro por Mes -->
    <div class="filter-section shadow-sm">
        <div class="row align-items-end g-3">
            <div class="col-md-8">
                <div class="label-min">Búsqueda Rápida (NIT / Nombre / Recibo)</div>
                <div class="input-group input-group-sm">
                    <span class="input-group-text bg-white"><i data-lucide="search" size="14"></i></span>
                    <input type="text" id="inputBusqueda" class="form-control" placeholder="Escriba para filtrar recibos..." onkeyup="ejecutarFiltro()">
                </div>
            </div>
            <div class="col-md-4">
    ...
    <div class="table-responsive">
        <table class="tabla-alt" id="tablaRecibos">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                ?>
                <tr class="row-recibo" 
                    data-mes="<?= (int)date('m', strtotime($fecha)) ?>"
                    data-search="<?= htmlspecialchars(strtolower($nit . ' ' . $nombre . ' recibo-' . $folio)) ?>">
                    ...
                    <td class="doc-ref">RECIBO-<?= $folio ?></td>
                    <td class="text-end">
                        <span class="badge-status st-cobrada">COBRADA</span>
                        
                        <?php 
                        // URLs para el visor dual (Ingreso + Banco)
                        $urlI = "abrir_archivo.php?file=" . urlencode(($r['ruta_carpeta'] ?? '') . ($r['egreso_path'] ?? ''));
                        ...
                        <button type="button" class="btn btn-link p-0 ms-2 text-dark" 
                                onclick="abrirVisorDual('<?= addslashes($urlI) ?>', '<?= addslashes($urlS) ?>')" 
...
<!-- Modal Visor Dual (Igual al Dashboard/Egresos) -->
    ...
    <div class="modal-dialog modal-xl modal-xl-custom modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4" style="background-color: #fcfcfc;">
            <div class="modal-header border-0 pb-0 p-4">
                <div>
                    <h5 class="modal-title fw-bold text-dark"><i data-lucide="files" class="me-2"></i>Visor de Recibos ALT</h5>
                    <p class="text-muted small mb-0">Comprobante de Ingreso vs Soporte de Banco</p>
                </div>
            ...
            <div class="modal-body p-4 pt-2">
                <div class="row g-3">
                    <div class="col-md-6">
                        <span class="doc-label">COMPROBANTE DE INGRESO</span>
                        <iframe id="iframeIngreso" class="doc-viewer w-100"></iframe>
                    </div>
...
<script>
    ...
    function abrirVisorDual(i, b) {
        document.getElementById('iframeIngreso').src = i;
    ...
    document.getElementById('modalVisor').addEventListener('hidden.bs.modal', function () {
        document.getElementById('iframeIngreso').src = '';
    ...
    function ejecutarFiltro() {
        ...
        const rows = document.querySelectorAll('.row-recibo');

...
$sql = "SELECT id, fecha_emision, archivo_path, soporte_pago_path, egreso_path, ruta_carpeta, estado FROM facturas_venta ORDER BY id DESC";
...
<main class="main-content p-4" style="margin-left: 280px; background-color: #f8f9fa;">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <div>
                <h2 class="fw-bold text-dark mb-1">Gestión de Ventas</h2>
                <p class="text-muted small mb-0">Listado de facturas de venta y recaudos almacenados.</p>
            </div>
            <div class="d-flex gap-2">
                ...
                <a href="cuentas_cobro.php" class="btn btn-dark shadow-sm px-4 fw-bold">RECAUDAR CUENTAS</a>
            </div>
        </div>

        <div class="row g-4" id="gridVentas">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <span class="badge rounded-pill px-3 py-2 <?= $esPagada ? 'bg-dark text-white' : 'bg-light text-dark border' ?>">
                                <?= $esPagada ? 'COBRADA' : strtoupper($f['estado']) ?>
                            </span>
                            <span class="text-muted small fw-medium"><?= $f['fecha_emision'] ?></span>
                        </div>

                        <h5 class="fw-bold mb-4 text-dark">ALT-VENTA-<?= $numReal ?></h5>
                        ...
                        <div class="d-grid mt-auto">
                            <?php if(!$esPagada): ?>
                                <a href="cuentas_cobro.php?id=<?= $f['id'] ?>" class="btn btn-dark rounded-pill fw-bold py-2">
                                    <i data-lucide="dollar-sign" size="16" class="me-2"></i> Procesar Recaudo
                                </a>
                            <?php else: ?>

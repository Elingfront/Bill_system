...
// Consulta a la tabla de ventas con JOIN a entidades
$sql = "SELECT f.id, f.fecha_emision, f.archivo_path, e.nit_cedula, e.nombre, f.soporte_pago_path, f.egreso_path, f.ruta_carpeta, f.estado, e.telefono, e.correo, e.direccion
...
<main class="main-content">
    <div class="alt-page-header">
        <div class="alt-header-content">
            <div class="alt-header-icon"><i data-lucide="trending-up" size="24"></i></div>
            <div>
                <h4 class="alt-header-title">Facturas de Venta</h4>
                <span class="alt-header-subtitle">ALT-CONFECCIONES · Control de Ingresos</span>
            </div>
        </div>
    ...
    <div class="filter-section shadow-sm">
        <div class="row align-items-end g-3">
            <div class="col-md-4">
                <div class="label-min">Búsqueda Rápida (NIT / Nombre / Factura)</div>
            ...
            <div class="col-md-3">
                <div class="label-min">Estado de Cobro</div>
            ...
            <div class="col-md-2">
                <a href="cuentas_cobro.php" class="btn btn-dark btn-sm w-100 fw-bold">
                    <i data-lucide="trending-up" size="14"></i> REGISTRAR COBRO
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="tabla-alt" id="tablaVentas">
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ($facturas as $f): 
                    $parts = explode('FACT_', $f['ruta_carpeta']);
                    $folio = isset($parts[1]) ? rtrim($parts[1], DIRECTORY_SEPARATOR . '/') : $f['id'];
                ...
                <tr class="row-factura" 
                    ...
                    data-mes="<?= (int)date('m', strtotime($f['fecha_emision'])) ?>"
                    data-search="<?= htmlspecialchars(strtolower($nit . ' ' . $nombre . ' venta-' . $folio)) ?>">
                    <td class="nit-font"><?= htmlspecialchars($nit) ?: '---' ?></td>
                    <td class="text-uppercase">
                        <strong style="font-size: 13px;"><?= htmlspecialchars($nombre) ?></strong>
                    </td>
                    <td class="fw-bold text-muted"><?= ($f['fecha_emision']) ? date('d/m/Y', strtotime($f['fecha_emision'])) : '---' ?></td>
                    <td class="doc-ref">VENTA-<?= $folio ?></td>
                    <td class="text-end">
                        <span class="badge-status <?= $esPagada ? 'st-pagada' : 'st-pendiente' ?>">
                            <?= $esPagada ? 'COBRADA' : 'PENDIENTE' ?>
                        </span>
                        
                        <?php 
                        $urlF = "abrir_archivo.php?file=" . urlencode(($f['ruta_carpeta'] ?? '') . ($f['archivo_path'] ?? ''));
                        $urlS = ($f['soporte_pago_path'] ?? '') ? "abrir_archivo.php?file=" . urlencode($f['ruta_carpeta'] . $f['soporte_pago_path']) : "";
                        $urlE = ($f['egreso_path'] ?? '') ? "abrir_archivo.php?file=" . urlencode($f['ruta_carpeta'] . $f['egreso_path']) : "";
                        ?>
                        
                        <button type="button" class="btn btn-link p-0 ms-2 text-dark" 
                                onclick="abrirVisorProfesional('<?= addslashes($urlF) ?>', '<?= addslashes($urlS) ?>', '<?= addslashes($urlE) ?>')" 
                                title="Ver Soportes Completos">
                            <i data-lucide="layout-grid" size="16"></i>
                        </button>

                        <?php if(!$esPagada): ?>
                            <a href="cuentas_cobro.php?id=<?= $f['id'] ?>" class="btn btn-link p-0 ms-2 text-vinotinto" title="Procesar Cobro">
    ...
    <div class="modal-dialog modal-xl modal-xl-custom modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4" style="background-color: #fcfcfc;">
            <div class="modal-header border-0 pb-0 p-4">
                <div>
                    <h5 class="modal-title fw-bold text-dark"><i data-lucide="files" class="me-2"></i>Visor de Ventas ALT</h5>
                    <p class="text-muted small mb-0">Factura de Venta, Soporte de Banco e Ingreso</p>
                </div>
            ...
            <div class="modal-body p-4 pt-2">
                <div class="row g-3">
                    <div class="col-md-4">
                        <span class="doc-label">FACTURA DE VENTA</span>
                    ...
                    <div class="col-md-4" id="colEgreso">
                        <span class="doc-label">COMPROBANTE INGRESO</span>
...
<script>
    ...
    function abrirVisorProfesional(f, s, e) {
    ...
    function ejecutarFiltro() {
        ...
        rows.forEach(row => {
            ...
            if (matchSearch && matchEstado && matchMes) {
    ...
    document.addEventListener('DOMContentLoaded', ejecutarFiltro);
</script>
